...
```
☞ **Exception**은 모든 에러 객체의 최고 조상님이다 <br> 근데 Exception이 다른 오류처리 구문보다 먼저 오게 되면 Exception 혼자 다 처리 가능하기 때문에 <br> 다른 오류처리 구문이 필요없다 그래서 하위 객체의 오류처리를 타지않고 그대로 구문으르 탈출! <br> 근데 오류처리를 하라고 하니 error가 발생하는 것  
