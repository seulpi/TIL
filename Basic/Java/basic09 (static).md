...
1. static인 이유는 메인메소드가 인스턴스 생성과 관계없이 가장 먼저 생성되기 때문
2. public인 이유는 메인은 외부호출이고 다른 패키지에서 언제어디서든 호출 가능해야하기때문
...
3. 인스턴스 변수와 다름 (인스턴스변수 = 인스턴스가 생성되었을때 vs 클래스변수 = 클래스가 메모리에 올라갔을때)
<br> - 인스턴스 변수는 인스턴스를 생성해야 사용이 가능하지만 클래스 변수는 생성하지 않아도 사용이 가능 
<br> - 인스턴스 변수에서 공통적으로 사용하는 변수라면 클래스 변수로 생성해 사용하는것도 설계시에 고려해 볼 것
